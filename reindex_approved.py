...
def main():
    ...
    category_map = {
        ...
        "memberships_leadership": "membership",
        "volunteer_community_service": "volunteer",
        "awards": "awards"
    }
    ...
    for path in approved_files:
        ...
        category = category_map.get(path_obj.stem, "unknown")
        ...
        for i, chunk in enumerate(chunks):
            ...
            all_chunks.append({
                "id": doc_id,
                "text": chunk,
                "metadata": {"source": path_obj.name, "filename": path_obj.name, "chunk_index": i, "category": category}
            })
            total_chunks += 1

        print(f"     Indexed {len(chunks)} chunks from {path_obj.name} (category: {category})")
